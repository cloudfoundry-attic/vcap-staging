   ...
   describe "when bundled" do
    ...
    describe "and using Ruby 1.8" do
      it "is packaged with a startup script" do
        stage({:framework => {:name => "standalone"},:meta=>{:command=> "ruby app.rb"}, :runtime=> ruby18_runtime}) do |staged_dir|
...
gem "cf-autoconfig"
    ...
    describe "and using Ruby 1.9" do
      it "is packaged with a startup script" do
        stage({:framework => {:name => "standalone"},:meta=>{:command=> "ruby app.rb"}, :runtime=> ruby19_runtime}) do |staged_dir|
          start_script = File.join(staged_dir, 'startup')
          start_script.should be_executable_file
          script_body = File.read(start_script)
          script_body.should == <<-EXPECTED
...
gem "cf-autoconfig"
  ...
  describe "when using Ruby and not bundled" do
    ...
    describe "and using Ruby 1.8" do
      it "is packaged with a startup script" do
        stage({:framework => {:name => "standalone"}, :meta=>{:command=> "ruby hello.rb"}, :runtime=> ruby18_runtime}) do |staged_dir|
          start_script = File.join(staged_dir, 'startup')
          start_script.should be_executable_file
          script_body = File.read(start_script)
          script_body.should == <<-EXPECTED
...
wait $STARTED
    ...
    describe "and using Ruby 1.9" do
      it "is packaged with a startup script" do
        stage({:framework => {:name => "standalone"}, :meta=>{:command=> "ruby hello.rb"}, :runtime=> ruby19_runtime}) do |staged_dir|
          start_script = File.join(staged_dir, 'startup')
          start_script.should be_executable_file
          script_body = File.read(start_script)
          script_body.should == <<-EXPECTED
...
wait $STARTED
  ...
  describe "with Java runtime" do
    ...
    it "is packaged with a startup script" do
      stage({:framework => {:name => "standalone"}, :meta=>{:command=> "java $JAVA_OPTS HelloWorld"}, :runtime=> {:name => "java"}, :resources=>{:memory=>512}}) do |staged_dir|
          start_script = File.join(staged_dir, 'startup')
          start_script.should be_executable_file
          script_body = File.read(start_script)
          script_body.should == <<-EXPECTED
...
wait $STARTED
  ...
  describe "with Python runtime" do
    ...
    it "is packaged with a startup script" do
      stage({:framework => {:name => "standalone"}, :meta=>{:command=> "python HelloWorld.py"}, :runtime=> {:name => "python2"}, :resources=>{:memory=>512}}) do |staged_dir|
          start_script = File.join(staged_dir, 'startup')
          start_script.should be_executable_file
          script_body = File.read(start_script)
          script_body.should == <<-EXPECTED
