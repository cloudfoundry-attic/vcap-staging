...
DROPLET_BASE_DIR=$PWD
cd app
#{executable} ./rubygems/ruby/1.8/bin/bundle exec #{executable} -rcfautoconfig -S ./rubygems/ruby/1.8/bin/rackup $@ config.ru -E $RACK_ENV > $DROPLET_BASE_DIR/logs/stdout.log 2> $DROPLET_BASE_DIR/logs/stderr.log &
STARTED=$!
echo "$STARTED" >> $DROPLET_BASE_DIR/run.pid
wait $STARTED
      EXPECTED
