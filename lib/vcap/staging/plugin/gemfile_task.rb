...
class GemfileTask
  ...
  def update_gemspec(olg_gemspec, new_gemspec)
    `cp #{new_gemspec} #{olg_gemspec}`
    @logger.error "Failed updating gemspec #{olg_gemspec}" if $?.exitstatus != 0
  end
  ...
  def build_gem(spec_path)
    ...
    gem_path = nil
    Dir.chdir(base) do
      cmd = "#{@ruby_cmd} -S gem build '#{spec_path}' --force -q"
      secure_file(base)
      run_secure(cmd, base)
      unsecure_file(base)
