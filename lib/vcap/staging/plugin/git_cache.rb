...
class GitCache
  ...
  def get_source(options, dst_dir)
    return unless options["uri"] && options["revision"]
    ...
    exitstatus = run_cmd_with_retry("git clone #{cached_path} #{dst_dir}")
    return if exitstatus != 0
    Dir.chdir(dst_dir) do
      `git reset --hard #{revision}`
      return if $?.exitstatus != 0
      `git submodule update --init --recursive` if (options["submodules"])
    end
    dst_dir
  end
  ...
  def update_git_cache(uri)
    ...
    begin
      ...
      FileUtils.remove_entry_secure(cache_rename_dir)
    ensure
  ...
  def create_git_cache_source_dir(where, source)
    tmp_dir = Dir.mktmpdir
    begin
      `git clone --mirror #{source} #{tmp_dir}`
