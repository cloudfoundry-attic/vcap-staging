...
class Rails3Plugin < StagingPlugin
  ...
  def precompile_assets_command
    opts = ""
    opts << " -rcfautoconfig" if autoconfig_enabled?
    if uses_bundler?
      "#{local_runtime} #{gem_bin_dir}/bundle exec #{local_runtime}#{opts} #{gem_bin_dir}/rake assets:precompile"
    else
      "#{local_runtime} -S#{opts} rake assets:precompile"
    end
  end
...
if [ ! -f "$PWD/app/public/assets/manifest.yml" ] ; then
  cd app
  #{precompile_assets_command} &>>../logs/assets_compilation.log
  cd ..
fi
      ASSETS_COMPILATION
      cmds << <<-RUBY_CONSOLE
